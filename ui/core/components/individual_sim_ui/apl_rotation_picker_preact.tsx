...
export const APLRotationPickerPreact = (props: AplRPProps) => {
	const [prepullActions, setPrepullActions] = useState(props.modPlayer.aplRotation.prepullActions)
    
    return (
        <div class='apl-rotation-picker-root'>
            <ListPickerPreact 
				...
				onCopyClicked={(srcIdx) => {
					...
					setPrepullActions(prepullActions);
					props.modPlayer.rotationChangeEmitter.emit(TypedEvent.nextEventID());
				}}
				onNewItem={()=> {
					prepullActions.push(APLPrepullAction.create({
						action: {},
						doAtValue: {
				...
				onItemDropped={(srcIdx, dstIdx) => {
					const arrElem = prepullActions[srcIdx];
					prepullActions.splice(srcIdx, 1);
					prepullActions.splice(dstIdx, 0, arrElem);
					setPrepullActions(prepullActions);
					props.modPlayer.rotationChangeEmitter.emit(TypedEvent.nextEventID());
				}}
				onDeleteClicked={(srcIdx) => {
					prepullActions.splice(srcIdx, 1);
					setPrepullActions(prepullActions);
					props.modPlayer.rotationChangeEmitter.emit(TypedEvent.nextEventID());
				}}
