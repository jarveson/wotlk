...
function element<T extends ((props: object) => TSXJSX.Element)>(component: T, props?: Parameters<T>[0] | null, ...children: TSXJSX.Children): ReturnType<T>

function element<T extends keyof MathMLTags>(tagName: T, props?: TSXJSX.MathMLElementProps<T> | null, ...children: TSXJSX.Children): MathMLElement

function element<T extends keyof SVGTags>(tagName: T, props?: TSXJSX.SVGElementProps<T> | null, ...children: TSXJSX.Children): SVGElementTagNameMap[T]

function element<T extends keyof HTMLTags>(tagName: T, props?: TSXJSX.HTMLElementProps<T> | null, ...children: TSXJSX.Children): HTMLElementTagNameMap[T]

function element(tagName: ((props: object) => TSXJSX.Element) | string, props?: { [key: string]: any } | null, ...children: TSXJSX.Children) {
	if (typeof tagName == "string") {
		...
		if (props) {
			...
			for (let prop in props) {
				if (!specialProps.has(prop)) (el as any)[prop] = props[prop]
			}
...
const fragment = ({ children }: { children?: TSXJSX.Child | TSXJSX.Children }) => {
...
const addStyles = (style: CSSStyleDeclaration, styles: TSXJSX.CSSProperties) => {
	for (let name in styles) {
		if (name.indexOf("--")) (style as any)[name] = (styles as any)[name]
		else style.setProperty(name, (styles as any)[name])
	}
}

const appendChildren = (children: TSXJSX.Children, parent: ParentNode) => {
...
/** Adds SVG support. Must be called before creating any SVG elements with TSXJSX. */
const addSVGSupport = () => {
...
/** Adds MathML support. Must be called before creating any MathML elements with TSXJSX. */
...
declare global {
	namespace TSXJSX {
		// The typing that can be done with TSXJSX.Element is unfortunitely very limited
		// TSXJSX.Element can also be a DocumentFragment which can cause problems
		...
		interface WritableElementProps extends HTMLTags {
			...
			button: {
				...
				popoverTargetElement: TSXJSX.Element
			...
			sup: {}
			table: {
				caption: TSXJSX.Element
				tHead: TSXJSX.Element
				tFoot: TSXJSX.Element
				/** @deprecated */
				align: Align
				/** @deprecated */
				bgColor: string
...
export { element, fragment, ref, appendChildren, addSVGSupport, addMathMLSupport, TSXJSX }